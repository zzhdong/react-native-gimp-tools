...
const modes = {
    hex: {
        getString: color => tinyColor2(color).toHexString(),
        label: 'HEX'
    },
    hsl: {
        getString: color => tinyColor2(color).toHslString(),
        label: 'HSL'
    },
    hsv: {
        getString: color => tinyColor2(color).toHsvString(),
        label: 'HSV'
    },
    rgb: {
        getString: color => tinyColor2(color).toRgbString(),
        label: 'RGB'
    }
};
...
export class SlidersColorPicker extends Component {
    constructor(props) {
        super(props);
        const color = tinyColor2(this.props.color).toHsl();
        const mode = tinyColor2(this.props.mode).toHsl();
        this.state = {
            color,
            mode: 'hex'
        };
    ...
    componentWillReceiveProps(nextProps) {
        this.setState({color: tinyColor2(nextProps.color).toHsl()});
    }
    ...
    render() {
        ...
        const colorHex = tinyColor2(this.state.color).toHexString();
        return (
            <Modal
                animationType="slide"
                transparent={false}
                        ...
                        <View style={styles.swatchesContainer}>
                            {swatches.map((swatch, index) => (
                                <TouchableOpacity
                                    ...
                                    onPress={() =>
                                        this.setState({color: tinyColor2(swatch).toHsl()})
                                    }
                                />
                            ))}
                        </View>
